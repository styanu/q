...
================================================================================
 ...
 【青龙配置】
   ...
   【多账号】同一个 STAR_RAIL_COOKIE 变量里放多个账号的 Cookie，用 & 或换行
      分隔，脚本会逐个账号、逐个角色签到，结果汇总成一条通知；单个账号失效
      不影响其他账号（重复的 Cookie 会自动去重）。
      推荐用 & 连接（青龙变量单行即可）：
        账号1的整段Cookie & 账号2的整段Cookie & 账号3的整段Cookie
      也可以一行一个账号（变量值里直接换行）：
        账号1的整段Cookie
        账号2的整段Cookie

 【获取 Cookie（电脑 Chrome/Edge，推荐无痕窗口，约 2 分钟）】
   ※ 签到页 act.mihoyo.com 现在只显示“下载米游社APP”二维码、没有网页签到按钮；
     且 cookie_token 是 HttpOnly，控制台 document.cookie 读不到。
     请务必从 Network 的“请求标头”里复制，并按下面让浏览器替你带上 cookie_token：

   1. 无痕窗口打开米游社官网并完成登录（确认右上角显示自己昵称/头像）：
        https://www.miyoushe.com/sr/
   2. 同一窗口地址栏访问下面这个签到页，停留 5~10 秒（让账号 SDK 静默登录、
      给 mihoyo.com 域写入 cookie_token）：
   ...
   3. 再在地址栏直接访问这个接口地址：
      ...
      · 若返回的 JSON 里 "retcode":0 且 data.list 中有你的角色 → 登录态已生效
      · 若 "retcode":-100 → 回到第 2 步刷新签到页多等几秒，再访问本地址
   4. 按 F12 → Network（网络）→ 点列表里那条 getUserGameRolesByCookie 请求
      → Headers（标头）→ Request Headers（请求标头）→ 找到 Cookie: ，
      复制冒号后面的【整段值】（切勿用控制台 document.cookie，那里缺 token）
   5. 粘贴到环境变量 STAR_RAIL_COOKIE 中
   ※ 自检：复制到的内容里必须包含 “cookie_token=” 字段；只有 account_id / ltuid
     之类是不能签到的。若脚本提示 -100 登录失效，重新按上述步骤获取即可。
...
def run_for_account(index: int, cookie: str) -> str:
    ...
    # Cookie 字段预检：签到接口硬性需要 cookie_token（HttpOnly）+ 账号 ID
    has_cookie_token = ("cookie_token=" in cookie) or ("cookie_token_v2=" in cookie)
    has_uid = any(k in cookie for k in
                  ("account_id", "account_id_v2", "ltuid", "stuid"))
    if not has_cookie_token or not has_uid:
        if not has_cookie_token and not has_uid:
            problem = "缺少 cookie_token 和 account_id"
        elif not has_cookie_token:
            problem = ("缺少登录令牌 cookie_token（你复制的可能是控制台 document.cookie "
                       "或 Application 面板的内容，读不到 HttpOnly 令牌）")
        else:
            problem = "缺少账号 ID（account_id）"
        lines.append(
            "❌ 这段 Cookie 不能签到：" + problem + "。\n"
            "    请严格按脚本顶部【获取 Cookie】步骤：先登录米游社官网 → 访问签到页停留几秒 "
            "→ 地址栏打开 getUserGameRolesByCookie 接口 → 从 Network 里该请求的 "
            "Request Headers → Cookie 整段复制（必须能看到 cookie_token= 字段）。")
        return "\n".join(lines)
