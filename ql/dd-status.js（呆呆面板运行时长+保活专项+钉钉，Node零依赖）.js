 ...
 *    4. 钉钉自定义机器人通知（webhook 全部走环境变量，支持加签 / @人 / 仅异常推送）
 *
 ...
 *
 *  ============================ 命令行参数 ==================================
 *    无参数        查看状态并按环境变量决定是否推送钉钉（只读，不修改任何东西）
 *    --no-notify    本次不推送钉钉
...
const cfg = {
  ...
  ctyunKeywords: (ENV.CTYUN_TASK_KEYWORDS || 'ctyun,天翼云手机,云手机保活,云手机,保活')
...
async function collectDaidai(api) {
  ...
  if (d.tasks.length === 0 && docker.bin) {
    try {
      ...
      if (t.length) { d.source = d.source ? `${d.source}+Docker(sqlite)` : 'Docker(sqlite)'; d.tasks = t.map(normTask); }
    } catch { /* ignore */ }
  }
  if (d.envs.length === 0 && docker.bin) {
    try {
      const e = docker.envs();
      if (e.length) { d.source = d.source ? `${d.source}+Docker(sqlite)` : 'Docker(sqlite)'; d.envs = e; }
    } catch { /* ignore */ }
  }
  return d;
}
...
async function checkCtyun(d, api) {
  ...
  const matched = d.tasks.filter(isCtyunTask);
  if (matched.length === 0) {
  ...
  for (const t of matched) {
    const item = {
      ...
      lastExec: t.lastRunAt, lastRunMs: t.lastRunSec * 1000, nextExec: t.nextRunAt,
      panelStatus: runStatusText(t.lastRunStatus), outcome: 'unknown', detail: '',
    };
    ...
    if (!t.lastRunAt) {
      warn('该任务从未执行过，请确认呆呆调度正常');
      addIssue(LEVEL.WARN, '天翼云手机保活', `「${t.name}」从未执行`);
    ...
    // 最近日志结果（开放 API）；sqlite 兜底无日志通道
    let logText = '';
    let logMeta = null;
    if (api.alive && t.id !== undefined) logMeta = await api.latestLog(t.id);
    if (logMeta) logText = logMeta.content || '';
    const ana = analyzeCtyunLog(logText);
    item.outcome = ana.outcome;
    if (!logText) {
      warn('最近日志    : 暂不可读（任务未产生日志，或未配置具备 tasks 权限的开放 API）');
...
/* ============================= 主流程 ==================================== */
function parseArgs() {
  const a = { notify: true, json: false };
...
async function main() {
  ...
  if (cfg.ddAppKey && cfg.ddAppSecret) {
    ...
    info('未配置 DD_APP_KEY/DD_APP_SECRET，任务级数据走 Docker sqlite 兜底（推荐配置开放 API）。');
  }
