...
const issues = [];           // 全局问题清单 {level, scope, msg, taskId}
function addIssue(level, scope, msg, taskId = null) { issues.push({ level, scope, msg, taskId }); }
...
const cfg = {
  ...
  // 服务端临时错误（系统繁忙/超时/网关等）自动延迟重试一次：默认开启，设 AUTO_FIX_RETRY_TRANSIENT=false 关闭
  fixRetryTransient: ENV.AUTO_FIX_RETRY_TRANSIENT === undefined ? true : /^(1|true|yes|on)$/i.test(ENV.AUTO_FIX_RETRY_TRANSIENT),
  retryDelaySec: Number(ENV.AUTO_FIX_RETRY_DELAY_SEC || 60),       // 发现临时失败后先等待多久再触发（让服务端繁忙缓解，避免立刻猛重试）
  retryWaitSec: Number(ENV.AUTO_FIX_RETRY_WAIT_SEC || 75),         // 触发后轮询等待复检的总时长（保活一次约30秒）
  retryCooldownMin: Number(ENV.AUTO_FIX_RETRY_COOLDOWN_MIN || 10), // 同一任务两次自动重试的最小间隔（分钟），防止连环重试加剧风控
...
// —— 临时错误自动重试的冷却状态（跨进程持久化，避免连环重试加剧风控）——
cfg.retryStateFile = ENV.AUTO_FIX_RETRY_STATE_FILE || path.join(cfg.dataDir, '.ctyun_retry_cooldown.json');
function loadRetryCooldown() {
  try { return JSON.parse(fs.readFileSync(cfg.retryStateFile, 'utf8')) || {}; } catch { return {}; }
}
function saveRetryCooldown(obj) {
  try {
    fs.mkdirSync(path.dirname(cfg.retryStateFile), { recursive: true });
    fs.writeFileSync(cfg.retryStateFile, JSON.stringify(obj, null, 2), 'utf8');
  } catch { /* 冷却状态写不进不影响主流程，退化为每次都允许重试 */ }
}
function retryInCooldown(taskId) {
  const last = Number(loadRetryCooldown()[String(taskId)] || 0);
  return last > 0 && (Date.now() - last) < cfg.retryCooldownMin * 60000;
}
function markRetryNow(taskId) {
  const cd = loadRetryCooldown();
  cd[String(taskId)] = Date.now();
  saveRetryCooldown(cd);
}
...
const autoFixer = {
  ...
  // 3.5) 服务端临时繁忙：先延迟缓解，再触发一次，并轮询日志复检是否恢复
  async retryTransient(task, api) {
    const id = task.id;
    if (cfg.dryRun) return { success: true, dryRun: true, message: `将等待 ${cfg.retryDelaySec}s 后触发 #${id} 并复检日志` };
    // 记录冷却（无论结果，防止短时间反复重试）
    markRetryNow(id);
    // 触发前的日志基线：用于区分“旧的失败日志”和“重试产生的新日志”
    let baseText = '';
    try { baseText = ((await fetchTaskLog(task, api)).text) || ''; } catch { baseText = ''; }
    // 分段等待，避免一次性长睡无法观察
    let remain = Math.max(0, cfg.retryDelaySec);
    while (remain > 0) { const step = Math.min(10, remain); await sleepMs(step * 1000); remain -= step; }
    const trig = await this.triggerTask(task, api);
    if (!trig.success) return { success: false, message: `自动重试触发失败：${trig.message}` };
    // 轮询等待新日志出现并判定结果
    const deadline = Date.now() + Math.max(15, cfg.retryWaitSec) * 1000;
    let lastFail = '';
    let changed = false;
    while (Date.now() < deadline) {
      await sleepMs(12000);
      let text = '';
      try { text = ((await fetchTaskLog(task, api)).text) || ''; } catch { continue; }
      if (!changed && text === baseText) continue;   // 还是旧日志，任务可能在排队/启动
      changed = true;
      const ana = analyzeCtyunLog(text);
      if (ana.outcome === 'success') {
        const bits = [ana.accountLine, ana.deviceLine].filter(Boolean).join('，');
        return { success: true, message: `已自动重试并恢复（${bits || '全部账号保活完成'}）` };
      }
      if (ana.outcome === 'failed') lastFail = ana.reasons[0] || '仍失败';
    }
    if (lastFail) return { success: false, message: `已自动重试，但仍失败：${String(lastFail).slice(0, 100)}` };
    return { success: false, message: '已自动重试触发，但等待期内未读到明确结果，请稍后查看面板执行日志' };
  },
  ...
  async runAll(api) {
    ...
    for (const item of fixPlan) {
      ...
      const gateMap = {
        ...
        fixRestartOcr: cfg.fixRestartOcr, fixRetryTransient: cfg.fixRetryTransient,
      };
      ...
      try {
        switch (item.type) {
          ...
          case 'retryTransient': res = await this.retryTransient(item.payload.task, api); break;
      ...
      recordFix({ type: item.type, title: item.title, target: item.target, taskId: item.payload.task ? item.payload.task.id : null, ...res });
      enabled.push(item);
    }
...
// cron 合理性分析：识别畸形表达式与不利于保活的执行密度
// 返回 {valid, warns:[], gapMin: 两次执行的最小间隔分钟数(null=无法判断), perDay: 每日大约执行次数}
function analyzeCron(expr) {
  const out = { valid: true, warns: [], gapMin: null, perDay: null };
  if (!expr || !String(expr).trim()) {
    out.valid = false; out.warns.push('未设置定时规则，任务不会自动执行');
    return out;
  }
  const parts = String(expr).trim().split(/\s+/);
  if (parts.length !== 5) {
    out.valid = false;
    out.warns.push(`cron 应为「分 时 日 月 周」5 位，当前是 ${parts.length} 位（${expr}）`);
    return out;
  }
  const [min, hour] = parts;
  const minStarEvery = min === '*';                       // 分位为 *（每分钟）
  const mStep = min.match(/^\*\/(\d+)$/);                // 分位 */N
  const hourFixed = /^\d+$/.test(hour);                  // 时位是单个固定小时
  const hourStar = hour === '*';                         // 时位为 *（每小时）
  // 估算最小执行间隔与每日次数
  if (minStarEvery && hourStar) {
    out.gapMin = 1; out.perDay = 1440;
    out.warns.push(`「${expr}」=每分钟执行一次，一天 1440 次，频率过高，极易触发风控`);
  } else if (minStarEvery && hourFixed) {
    out.gapMin = 1; out.perDay = 60;
    out.warns.push(`「${expr}」=每天 ${hour}:00-${hour}:59 这一小时内每分钟跑一次（共60次）、其余时间不执行，通常是写错了：每天${hour}点整应写「0 ${hour} * * *」，每N分钟应写「*/N * * * *」`);
  } else if (mStep && hourStar) {
    out.gapMin = Number(mStep[1]); out.perDay = Math.floor(1440 / out.gapMin);
    if (out.gapMin < 5) out.warns.push(`每 ${out.gapMin} 分钟一次过于频繁，建议不低于 20 分钟以降低风控概率`);
  } else if (/^\d+$/.test(min) && hourStar) {
    out.gapMin = 60; out.perDay = 24;
  } else if (hourFixed) {
    out.gapMin = 1440; out.perDay = 1;
  }
  // 保活密度提示：间隔超过 6 小时，保活基本失效
  if (out.gapMin !== null && out.gapMin > 360) {
    out.warns.push(`相邻两次执行间隔约 ${(out.gapMin / 60).toFixed(0)} 小时，对“保活”来说过长，云手机可能早已掉线，建议每 20~30 分钟一次（*/20 或 */30）`);
  }
  return out;
}
...
function analyzeCtyunLog(text) {
  ...
  // 判定是否为「可重试的临时性错误」：服务端繁忙/超时/网关/网络抖动等；账号密码错误、锁定、验证码、无设备等确定性错误不重试
  if (r.outcome === 'failed') {
    const RETRIABLE = /系统繁忙|稍后再试|稍后重试|请重试|繁忙|超时|timeout|timed out|econnreset|econnrefused|etimedout|socket hang up|network|网络|网关|502|503|504|服务不可用|bad gateway|gateway|reset by peer|temporarily/i;
    const FATAL = /账号或密码错误|密码错误|已被锁定|被锁定|验证码|未查询到云手机|无权限|权限不足|devicecode|设备校验失败/i;
    const allText = r.reasons.join(' ') || text;
    const hasFatal = FATAL.test(allText);
    const hasRetriable = RETRIABLE.test(allText);
    r.transient = !hasFatal && hasRetriable;
  }
  return r;
}
...
async function checkCtyun(d, api) {
  ...
  for (const t of matched) {
    ...
    log(`  定时规则    : ${t.cron || '未设置'}`);
    // cron 合理性校验（只告警不自动改，执行频率需用户自行决定）
    const cronAna = analyzeCron(t.cron);
    if (cronAna.perDay !== null) log(`  每日约执行  : ${cronAna.perDay} 次${cronAna.gapMin ? `（最小间隔约 ${cronAna.gapMin} 分钟）` : ''}`);
    if (cronAna.warns.length > 0) {
      for (const w of cronAna.warns) {
        warn(`定时规则异常: ${w}`);
        addIssue(LEVEL.WARN, '天翼云手机保活', `「${t.name}」cron ${t.cron}：${w}`);
    ...
    if (t.timeKnown) {
      ...
      if (ps === '失败' || ps === '已终止') {
        error(`面板记录状态: ${ps}`);
        addIssue(LEVEL.ERROR, '天翼云手机保活', `「${t.name}」面板记录最后执行为${ps}`, t.id);
        result.conclusion = LEVEL.ERROR;
      } else if (ps !== '从未执行') {
        info(`面板记录状态: ${ps}`);
      }
    ...
    if (!logText) {
      ...
      addIssue(LEVEL.ERROR, '天翼云手机保活', `「${t.name}」最近一次日志失败${ana.reasons[0] ? '：' + ana.reasons[0] : ''}`, t.id);
      result.conclusion = LEVEL.ERROR;
      item.detail = ana.reasons.join('；');
      // 服务端临时繁忙/超时类错误：延迟后自动重试一次（带冷却，且任务当前不在运行中）
      if (ana.transient) {
        if (t.running) {
          info('该失败属于服务端临时繁忙，但任务当前仍在运行，不重复触发');
        } else if (retryInCooldown(t.id)) {
          info(`属于服务端临时繁忙，但 ${cfg.retryCooldownMin} 分钟内已自动重试过（冷却中），本次不再触发，仅告警`);
        } else {
          warn(`属于服务端临时繁忙/抖动，计划延迟 ${cfg.retryDelaySec}s 后自动重试一次并复检`);
          planFix('retryTransient', `服务端临时繁忙，延迟${cfg.retryDelaySec}s后自动重试一次`, `#${t.id} ${t.name}`, { task: t }, 'fixRetryTransient');
        }
      }
    } else {
...
async function main() {
  ...
  if (cfg.autoFix && fixPlan.length > 0) {
    ...
    if (!cfg.dryRun && fixResults.some((r) => r.success)) {
      ...
      // 自动重试已恢复的任务：撤销其对应的失败告警，避免总体结论误报
      const recovered = new Set(fixResults.filter((r) => r.type === 'retryTransient' && r.success && r.taskId != null).map((r) => r.taskId));
      if (recovered.size > 0) {
        for (let i = issues.length - 1; i >= 0; i -= 1) {
          if (issues[i].taskId != null && recovered.has(issues[i].taskId)) issues.splice(i, 1);
        }
        info('已自动重试恢复，撤销该任务原先的失败异常项（钉钉将不再就本次临时抖动告警）');
      }
    }
  } else {
    showFixResults();
