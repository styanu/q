...
# -*- coding: utf-8 -*-
"""
老王论坛 自动签到脚本（域名可通过环境变量切换）
适配：呆呆面板 / 青龙面板 / 通用 Python 环境

【站点说明】
  Discuz! + k_misign 签到插件，登录和签到各需一次滑块验证码
  脚本自动破解滑块验证码，无需人工干预
  支持老王论坛所有镜像域名（laowang3ih2so.com / laowangh5ntce.com 等）

【使用方法】
  在面板中添加环境变量：
    LWH5_BASE_URL = 站点网址（可选，默认 https://laowangh5ntce.com，换域名只需改这个）
    LWH5_USERNAME = 你的用户名
    LWH5_PASSWORD = 你的密码
  多账号用 | 分隔，例如：
    LWH5_USERNAME = user1|user2
    LWH5_PASSWORD = pass1|pass2

  示例（切换到老域名）：
    LWH5_BASE_URL = https://laowang3ih2so.com
    LWH5_USERNAME = msksegrehdbfvdc
    LWH5_PASSWORD = 88888888@qq.com
...
# 配置区
# ============================================================
# 域名可通过环境变量 LWH5_BASE_URL 配置，默认使用 laowangh5ntce.com
BASE_URL = os.environ.get("LWH5_BASE_URL", "https://laowangh5ntce.com").rstrip("/")
...
# ============================================================
def main():
    domain_source = "环境变量" if os.environ.get("LWH5_BASE_URL") else "默认"
    log(f"老王论坛自动签到脚本启动（域名来源: {domain_source}）")
    log(f"目标站点: {BASE_URL}")

    env_username = os.environ.get("LWH5_USERNAME", "") or os.environ.get("lwh5_username", "")
    env_password = os.environ.get("LWH5_PASSWORD", "") or os.environ.get("lwh5_password", "")

    if not env_username or not env_password:
        log("未检测到有效配置！", "ERROR")
        log("请设置环境变量：LWH5_USERNAME 和 LWH5_PASSWORD", "INFO")
        sys.exit(1)
