...
======================== 青龙环境变量 ========================
【多账号 token（必填）】二选一：
  YZ_TOKENS   : 多账号，每行一个；脚本会用正则自动抠出其中的 JWT，因此：
                可直接贴纯 JWT、"备注:JWT"，甚至整段
                "channel_code=...; CG_CLINET_USER_TOKEN_YUN=eyJ..." 也能识别。
  YZ_TOKEN    : 单账号时可只填这一个。
  例（YZ_TOKENS，多个用换行）：
     eyJhbGciOi....（账号A的 JWT）
     小号:eyJhbGciOi....（账号B）
  非法/被截断的 token 会被跳过并在报告中明确列出，不会误跑成“登录失效”。

【钉钉机器人（可选）】两套变量名二选一，脚本自动识别：
  ① DINGTALK_WEBHOOK : 机器人完整 Webhook（含 access_token=...）
     DINGTALK_SECRET  : 机器人「加签」密钥（SEC 开头）
  ② DD_BOT_TOKEN      : Webhook 里 access_token= 后面那串
     DD_BOT_SECRET     : 机器人「加签」密钥
  机器人安全设置请选「加签」；不配置则只在青龙日志输出、不推送。
...
def dingtalk_config():
    """兼容两种常见变量命名：
       ① DINGTALK_WEBHOOK(完整 webhook) + DINGTALK_SECRET(加签密钥)
       ② DD_BOT_TOKEN(access_token)      + DD_BOT_SECRET(加签密钥)
       返回 (access_token, secret)。"""
    webhook = (os.getenv("DINGTALK_WEBHOOK", "") or
               os.getenv("DD_BOT_WEBHOOK", "")).strip()
    token = ""
    if webhook:
        try:
            q = parse_qs(urlparse(webhook).query)
    ...
    secret = (os.getenv("DINGTALK_SECRET", "") or
              os.getenv("DD_BOT_SECRET", "")).strip()
    return token, secret


def dingtalk_notify(content):
    token, secret = dingtalk_config()
    if not token:
        return False, ("未配置钉钉机器人，跳过推送（可设置 DINGTALK_WEBHOOK+DINGTALK_SECRET，"
                       "或 DD_BOT_TOKEN+DD_BOT_SECRET）")
...
# ========================= 多账号解析 =========================
# JWT 形如 xxxxx.yyyyy.zzzzz，header 以 eyJ 开头；用正则从任意粘贴文本（cookie 串、
# “备注:”、“名字=”、空格/换行包裹等）中直接抠出 token，避免把 cookie 名/标点当成 token。
JWT_RE = re.compile(r"eyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+")
_COOKIE_NAME_RE = re.compile(
    r"(CG_CLINET_USER_TOKEN_YUN|CG_CLIENT_USER_TOKEN_YUN|access_token|authorization|token)[:=]*$",
    re.IGNORECASE)


def validate_jwt(tok):
    """返回 (ok, reason)。HS256 的 JWT 签名为 32 字节 -> base64url 固定 43 字符，
    可据此识别被截断/复制不全的 token。"""
    parts = tok.split(".")
    if len(parts) != 3 or not all(parts):
        return False, "不是合法 JWT（应为三段且均非空）"
    jwt = decode_jwt(tok)
    if not jwt or not (jwt.get("sub") or jwt.get("userId")):
        return False, "无法从 token 解析出账号 userId"
    sig_len = len(parts[2])
    if sig_len != 43:
        return False, "token 签名段长度异常（{} 字符，应为 43，通常是复制时被截断，请重新完整复制）".format(sig_len)
    return True, ""


def _alias_from_line(line, start):
    """从 token 之前的文本里提取备注，如 “小号A:eyJ...” -> “小号A”。"""
    before = line[:start]
    before = re.split(r"[\n,;]+", before)[-1]          # 取最近一段
    before = before.strip().split()[-1] if before.strip() else ""
    before = before.strip().strip(":：=").strip()
    before = _COOKIE_NAME_RE.sub("", before).strip().strip(":：=").strip()
    if len(before) > 20:                                # 过长多半是 URL/杂串，不当备注
        before = ""
    return before


def parse_accounts():
    """从 YZ_TOKENS / YZ_TOKEN 提取账号。
    返回 (accounts, malformed)：accounts=[(备注,token),...]，malformed=[(标识,原因),...]。"""
    raw = (os.getenv("YZ_TOKENS", "") or "").strip() or \
          (os.getenv("YZ_TOKEN", "") or "").strip()
    accounts, malformed, seen = [], [], set()
    if not raw:
    ...
    for line in raw.splitlines():
        for m in JWT_RE.finditer(line):
            ...
            if tok in seen:
                continue
            ok, reason = validate_jwt(tok)
            if not ok:
                tag = _alias_from_line(line, m.start()) or "非法项"
                malformed.append((tag, reason))
                continue
            seen.add(tok)
            accounts.append((_alias_from_line(line, m.start()), tok))
    return accounts, malformed


def main():
    accounts, malformed = parse_accounts()
    today = time.strftime("%Y-%m-%d", time.localtime())

    # 非法/被截断的 token：明确列出，避免被当成“登录失效”误判
    bad_block = []
    if malformed:
        bad_block.append("==== 配置异常（以下 token 已跳过，未执行） ====")
        for tag, reason in malformed:
            bad_block.append("✗ {}：{}".format(tag, reason))
        bad_block.append("提示：请只填 JWT 本体（eyJ 开头、以签名段结尾），"
                         "多账号用换行分隔；不要把整段 cookie 或“CG_CLINET_USER_TOKEN_YUN=”一起贴入。")

    if not accounts:
        report = "{} 云智手机每日领奖（配置错误 ⚠️）\n\n{}".format(
            today, "\n".join(bad_block) if bad_block
            else "未检测到有效账号 token，请在青龙环境变量配置 YZ_TOKENS（或 YZ_TOKEN）。")
        print("\n" + report + "\n")
    ...
    if malformed:
        flags.append("有配置异常项")
    if any_auth_fail:
        flags.append("有账号登录失效")
    suffix = "（{} ⚠️）".format("，".join(flags)) if flags else ""
    title = "{} 云智手机每日领奖{}".format(today, suffix)
    report = title + "\n\n" + "\n\n".join(blocks)
