console.log(`安装nodejs依赖 axios socks-proxy-agent@8 smallfawn@latest三个`);

//设置中国时间
process.env.TZ = "Asia/Shanghai";
let signApiUrls = [];
let banUserId = [];
let ksmaxtask_look = process.env["ksmaxtask_look"] || 50;
let ksmaxtask_food = process.env["ksmaxtask_food"] || 5;
let ksmaxtask_box = process.env["ksmaxtask_box"] || 5;
let ksmaxtask_search = process.env["ksmaxtask_search"] || 25;
let searchKey = "";
let signApi = "";
let ksnoDelay = process.env["ksnoDelay"] || "false";
let ksmaxreward = process.env["ksmaxreward"];
let ksTaskNum = process.env["ksTaskNum"];
...
let kstask = process.env["kstask"] || "look,box,search";
let ksdailytask = process.env["ksdailytask"];
let task = [];
let invite = [];
let invite2 = [];
let ksuserinvite = process.env["ksuserinvite"] || [];
...
class Task {
  constructor(env) {
    ...
    // 新增：是否追加广告控制
    this.isAdAddEnabled = ksisadadd !== false; // 默认true，当设置为false时不追加广告
    this.apiac = "";
    // 新增：当前收益属性
    this.currentEarnings = 0;
    // 新增：最大金币限制
    this.maxReward = 30 * 1000; // 默认0表示无限制
    if (ksmaxreward) {
      try {
        this.maxReward = parseInt(ksmaxreward);
        if (isNaN(this.maxReward) || this.maxReward < 0) {
          this.maxReward = 30 * 1000;
        }
      } catch (e) {
        this.maxReward = 30 * 1000;
      }
  ...
  checkCookieVariables() {
    const variablesToCheck = [
      "kpn", "kpf", "userId", "did", "c", "appver", "language",
      "mod", "did_tag", "egid", "oDid", "androidApiLevel", "newOc",
      "browseType", "socName", "ftt", "abi", "userRecoBit", "device_abi",
      "grant_browse_type", "iuid", "rdid", "kuaishou.api_st"
    ];
    ...
    if (cookies) {
      ...
      // 识别并移除 SMPM 参数
      if (cookieObj.hasOwnProperty("SMPM")) {
        $.log(`账号[${this.index}] 检测到 SMPM 参数，防黑模式启动！`);
      ...
      for (const [key, rawValue] of Object.entries(defaultValues)) {
        if (!cookieObj.hasOwnProperty(key)) {
          ...
          $.log(`账号[${this.index}] ${key}缺失，使用默认值: ${rawValue}`);
        }
      }

      // 重构 CK 字符串 (更新 this.ck)
      this.ck = Object.entries(cookieObj)
        .map(([k, v]) => `${k}=${v}`)
        .join("; ");
    }
  ...
  getOaid() {
    ...
    return cookieObj.oaid || "93ece41c64ee5262";
  }
  ...
  async run() {
    ...
    //从searchKey随机抽取
    searchKey = kssearch[Math.floor(Math.random() * kssearch.length)];
  ...
  async taskInvite1(data) {
    try {
      ...
      let sig56_1 = await this.getSig56(
        Buffer.from(JSON.stringify(data)).toString("base64")
      );
      let options = {
        method: "POST",
...
async function testApi(url) {
  try {
    const { data: res } = await axios.get(url + '/ping', {
      timeout: 5 * 1000,
      headers: {
        referer: "https://smallfawn.top",
        "User-Agent":
...
!(async () => {
  ...
  // 实现：从signApiUrls中随机选择并测试，失败则选择下一个
  let availableSignApi = null;
  let shuffledUrls = [...signApiUrls].sort(() => Math.random() - 0.5); // 随机打乱数组
  //let shuffledUrls = signApiUrls;
  ...
  $.log(`🔄 广告追加模式: ${ksisadadd !== "false" ? "开启" : "关闭"}`);
  ...
  if (kstask) {
    try {
      task = kstask.split(",");
      console.log("读取到任务变量配置 " + JSON.stringify(task));
    } catch (e) {
      task = ["look", "food", "box"];
      $.log(`获取任务列表失败 请检查格式 执行默认全部执行food,box,look`);
    }
  } else {
    $.log(`未找到变量 kstask 使用默认任务 food,box,look`);
    task = ["look", "food", "box"];
  }
  if (kssearch) {
    try {
      kssearch = kssearch.split(",");
      console.log("读取到搜索光变量配置 " + JSON.stringify(kssearch));
    } catch (e) {
      kssearch = ["短剧", "好货", "百度极速版"];
      $.log(
        `获取任务列表失败 请检查格式 执行默认全部执行 ${JSON.stringify(
          kssearch
        )}`
      );
    }
    ...
    kssearch = ["短剧", "好货", "百度极速版"];
    $.log(`未找到变量 kssearch 使用默认任务 ${JSON.stringify(kssearch)}`);
  }
  if (ksdailytask) {
    try {
      ksdailytask = ksdailytask.split(",");
      console.log("读取到搜索广变量配置 " + JSON.stringify(ksdailytask));
    } catch (e) {
      ksdailytask = ["signin", "box", "huge"];
      $.log(
        `获取任务列表失败 请检查格式 执行默认全部执行 ${JSON.stringify(
          ksdailytask
        )}`
    ...
    ksdailytask = ["signin", "box", "huge"];
    $.log(`未找到变量 ksdailytask 使用默认任务 signin,box,huge`);
  }
  ...
  for (let i = 0; i < $.userList.length; i += concurrency) {
    userChunks.push($.userList.slice(i, i + concurrency));
  }
