...
功能：
  ...
  4. 输出简洁专业的 Markdown 报告（stdout + 可选写文件 + 青龙面板通知推送）
...
def build_report():
    ...
    if fut:
        out.append("**国内黄金价格**")
        out.append("")
        out.append("| 品种 | 最新价 | 涨跌 | 涨跌幅 |")
        out.append("|:--|:--:|:--:|:--:|")
        out.append(f"| 沪金主连（元/克） | {fut['last']:.2f} | {fut['chg']:+.2f} | **{fmt_pct(fut['pct'])}** |")
        if au9999:
            extra = f"（高 {au9999['high']:.2f} / 低 {au9999['low']:.2f}）" if au9999['high'] else ""
            out.append(f"| 上海金Au99.99（元/克） | {au9999['last']:.2f} {extra} | — | 现货参考 |")
        if autd:
            extra = f"（高 {autd['high']:.2f} / 低 {autd['low']:.2f}）" if autd['high'] else ""
            out.append(f"| 黄金T+D（元/克） | {autd['last']:.2f} {extra} | — | 现货参考 |")
        if xau:
            extra = f"（高 {xau['high']:.2f} / 低 {xau['low']:.2f}）" if xau['high'] else ""
            out.append(f"| 伦敦金现（美元/盎司） | {xau['last']:.2f} {extra} | — | 国际参考 |")
        out.append("")
    if etfs:
...
def main():
    ...
    # 可选：写入同目录文件
    try:
        path = sys.argv[1] if len(sys.argv) > 1 else "gold_report.txt"
    ...
    # 青龙通知推送（可选，失败自动忽略）
    try:
        qinglong_notify("国内黄金与黄金基金每日行情 " + datetime.now().strftime("%Y-%m-%d"), report)
    except Exception:
        pass
