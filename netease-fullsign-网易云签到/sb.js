...
const version = "20260805-smart-retry";
...
function switchSignApi() {
  ...
  console.log(`🔄 全局切换签名API节点：第 ${currentApiIndex + 1} 个`);
  return true;
}
...
class Task {
  constructor(envStr) {
    ...
    // 容错控制参数
    this.maxApiRetry = 2;       // 单个广告最多切换2次API（共尝试3个节点）
    this.stopAfterFail = 5;     // 连续失败5次自动停止该任务
    this.adFailCount = { look: 0, food: 0, box: 0, search: 0 };
    ...
    for (let retry = 0; retry <= this.maxApiRetry; retry++) {
      ...
      try {
        ...
        if (result.errorMsg === "OK" && result.feeds?.[0]?.ad) {
          ...
          // 签名失效错误，切换节点重试
          if (result.result === 40) {
            $.log(`🔄 检测到签名失效(错误码40)，尝试切换签名API节点`);
            const hasNext = switchSignApi();
            if (!hasNext) {
              $.log(`❌ 所有签名API节点均已失效，停止所有广告任务`);
              Object.keys(this.adTypesEnabled).forEach(k => this.adTypesEnabled[k] = false);
              this.shouldStop = true;
              this.stopReason = "所有签名API节点均失效";
              return null;
            }
            // 切换成功，等待后重试
  ...
  async runAdType(type) {
    ...
    for (let i = 1; i <= cfg.count; i++) {
      ...
      if (res === "success") {
        ...
        if (i < cfg.count) {
          const baseDelay = type === "look" ? randomInt(8, 12) : randomInt(10, 15);
          await $.wait(baseDelay * 1000);
        }
      } else {
        // 失败计数+降频
        this.adFailCount[type]++;
        const failTimes = this.adFailCount[type];
        
        if (failTimes >= this.stopAfterFail) {
          $.log(`⚠️ ${cfg.name}连续失败${failTimes}次，自动停止该任务，避免风控`);
          this.adTypesEnabled[type] = false;
          break;
        }
        
        // 失败后递增等待时间
        const extraWait = failTimes * 3;
        $.log(`⏳ 连续失败${failTimes}次，增加等待${extraWait}秒`);
        await $.wait((10 + extraWait) * 1000);
      }
    }
