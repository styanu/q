...
- 钉钉:  DINGTALK_WEBHOOK（必填） + DINGTALK_SECRET（加签时必填）
- 强制推送: FORCE_PUSH=1（忽略时间窗口立即推送）
"""
import os
import base64
import random
    ...
    for i2 in range(16):
        ...
        while True:
            ...
            if i5 >= 0:
                ...
                if i6 > 2147483647:
                    i6 = -4294967296 + i6
                # 核心修复：索引取低6位，防止Python大整数导致数组越界
                i6 |= static_f[i5][q[i5] & 0x3F]
                r = i6
                i4 = i5 - 1
            else:
                break
...
def get_accounts_from_env():
    ...
    if not env_value:
        return []
    accounts = []
    account_strings = env_value.splitlines()
...
def push_plus_send(title, content, force=False):
    ...
    if not token:
        print('⚠️ 未配置 PUSH_PLUS_TOKEN 环境变量，跳过PushPlus推送')
        return False
    try:
...
def dingtalk_send(title, content):
    """钉钉机器人推送，支持加签模式"""
    webhook = os.getenv('DINGTALK_WEBHOOK', '').strip()
    if not webhook:
        print('⚠️ 未配置 DINGTALK_WEBHOOK 环境变量，跳过钉钉推送')
        return False
    ...
    try:
        ...
        if result.get('errcode') == 0:
            ...
            print(f'❌ 钉钉推送失败: {err_msg}')
            if result.get('errcode') == 31000:
                print('  → 通常是机器人安全设置不匹配：请检查关键词/加签/IP白名单与配置是否一致')
            return False
    except Exception as e:
