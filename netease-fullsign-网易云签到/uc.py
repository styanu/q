...
# ========== 代理（SK5/SOCKS5/HTTP） ==========
# 全局加签锁：本地 SignRPC 一般是单进程服务，并发加签容易 429/超时，统一串行
SIGN_LOCK = threading.Lock()


def build_proxies(spec: str) -> Optional[Dict[str, str]]:
    """解析 SK5/SOCKS5/HTTP 代理为 requests 的 proxies 字典。

    支持写法：
      socks5://user:pass@host:port
      sk5://1.2.3.4:1080   （sk5/s5 视为 socks5 别名）
      1.2.3.4:1080          （纯 host:port 按 SOCKS5 处理）
      http://host:port
    """
    s = (spec or "").strip()
    if not s:
        return None
    low = s.lower()
    if low.startswith(("sk5://", "s5://")):
        s = "socks5://" + s.split("://", 1)[1]
    elif not low.startswith(("socks5", "socks4", "http://", "https://")):
        s = "socks5://" + s  # 纯 host:port 默认按 SOCKS5 处理
    return {"http": s, "https": s}


def _needs_socks(accounts: List[Dict[str, Any]]) -> bool:
    """判断是否需要 PySocks（用到 SOCKS 类代理时）"""
    specs = [acc.get("proxy") for acc in accounts if acc.get("proxy")]
    g = os.getenv("UC_PROXY", "").strip()
    if g:
        specs.append(build_proxies(g))
    for sp in specs:
        if sp and any(str(v).lower().startswith("socks") for v in sp.values()):
            return True
    return False
...
def log(msg: str, level: str = "信息") -> None:
    ...
    tag = "" if tname in ("MainThread", "Main") else f"[{tname}]"
    print(f"[{time.strftime('%H:%M:%S')}][{level}]{tag} {msg}", flush=True)
...
def parse_accounts(raw: str) -> List[Dict[str, Any]]:
    ...
    for line in (raw or "").splitlines():
        ...
        # 解析代理（每账号独立代理更利于防风控）
        proxy_spec = kv.get("proxy") or kv.get("sk5") or kv.get("socks") or kv.get("socks5")
        proxy = build_proxies(proxy_spec) if proxy_spec else None
        accounts.append({
            "label": label or user,
            "user": user,
            "token": tok,
...
# ========== Client（任务刷取 + 真实养号） ==========
class Client:
    def __init__(self, tok: Dict[str, Any], rpc_url: str, proxy: Optional[Dict[str, str]] = None) -> None:
        self.tok = tok
        self.kps = tok["kps"]
    ...
    def _stream_video(self, vurl: str, watch_seconds: int) -> None:
        ...
        try:
            ...
            resp = self.s.get(vurl, headers=headers, stream=True, timeout=30, verify=False)
...
def run_one_account(account: Any, rpc: str) -> Dict[str, Any]:
    ...
    if not proxy:
        gspec = os.getenv("UC_PROXY", "").strip()
        proxy = build_proxies(gspec) if gspec else None
    if proxy:
        log(f"账号 {label} 使用代理: {list(proxy.values())[0]}", "信息")
    client = Client(tok, rpc, proxy=proxy)
    res: Dict[str, Any] = {
        "user": user,
        "claims": [],
...
def main() -> int:
    ...
    # SOCKS5/SK5 代理依赖 PySocks
    if _needs_socks(accounts):
        try:
            import socks  # PySocks
        except Exception:
            log("检测到 SOCKS5/SK5 代理但未安装 PySocks，请执行: pip install PySocks", "错误")
            return 2

    # ========== 多账号并发 ==========
    max_workers = int(os.getenv("UC_MAX_WORKERS", "0") or "0")
    if max_workers <= 0:
        max_workers = min(len(accounts), 8)
    max_workers = min(max_workers, len(accounts))
    log(f"并发模式：{max_workers} 线程（账号数 {len(accounts)}）")
    log("提示：每账号可在 UC_ACCOUNTS 行内用 proxy= 指定独立 SK5 代理，更利于防风控")
