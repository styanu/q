...
附加功能（签到/宝箱/打卡）需 smallfawn 包，无包不影响核心广告任务
*/
...
let version = "20251105-opt-qinglong";
...
!(async () => {
  ...
  // 推送通知
  const notifyMsg = `【快手极速版任务】\n总账号: ${userEarnings.length}个\n总金币: ${grandTotal}\n预估收益: ${(grandTotal / 10000).toFixed(2)}元`;
  $.notifyStr.push("\n" + notifyMsg);

})()
  .catch(e => console.log("💥 全局异常:", e))
  .finally(() => $.done());