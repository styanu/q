...
const kssearch = process.env["kssearch"] || "";
...
const version = "20260805-qinglong-final";
...
class Task {
  ...
  async run() {
    ...
    // 每日任务
    if (ksdailytask.includes("signin")) await this.dailySignIn();
    if (ksdailytask.includes("box")) await this.openTreasureBox();
...
!(async () => {
  ...
  try { ksdailytask.split(","); } catch (e) { }
  if (kssearch) {
    try { kssearch.split(","); } catch (e) { }
  } else {
    kssearch = ["短剧", "好货", "美食", "穿搭"];
  }
